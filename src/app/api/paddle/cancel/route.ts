  ...
  try {
    ...
    return withRateLimit(userId, rateLimitLemonsqueezy, async () => {
      ...
      await db.execute({
        ...
        args: [crypto.randomUUID(), userId, JSON.stringify({ paddle_subscription_id: subscriptionId })],
      })
    ...
    console.error('Paddle cancel error:', e instanceof Error ? e.message : 'Unknown error')
